...
def calc_pdanger(psname):
    ...
    mycursor.close()
    mydb.close()
    pdanger=int(0 if pdanger is None else pdanger*2)
    return(pdanger)
...
pdanger_lo = fuzz.trapmf(x_pdanger, [0,0, 3, 5])
pdanger_md = fuzz.trapmf(x_pdanger, [3, 5, 6, 7])
pdanger_hi = fuzz.trapmf(x_pdanger, [5, 10, 20, 20])

missing_lo = fuzz.trapmf(x_missing, [0,0, 3, 5])
missing_md = fuzz.trapmf(x_missing, [3, 5, 6, 7])
missing_hi = fuzz.trapmf(x_missing, [5, 10, 20, 20])
...
report_md = fuzz.trapmf(x_report, [3, 5, 6, 7])
report_hi = fuzz.trapmf(x_report, [5, 10, 20, 20])
