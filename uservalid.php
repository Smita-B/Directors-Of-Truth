...
<p>
  (User's Validity Checking)
	...
	<?php
		$result2=mysqli_query($conn,"SELECT * FROM user_registration ");
		...
		?>
        					...
        					<td data-title='PHOTOTYPE'>
							<?php echo $row['PHOTOID_TYPE'] ?>
        					</td>
        					<td data-title='PHOTO'>
							<img src="<?php echo $PHOTO  ?>" height="50" width="50" alt=$PHOTO >
        					</td>
							<td data-title='NO'>
							<?php echo $row['PHOTOID_NO'] ?>
        					</td>
        					<td class='Grant Request'>
          					<a class='button' href='delete.php?fn=<?php echo $row["REG_ID"] ?> & table=user_registration'>
							<CENTER>Delete Profile</CENTER>
          					</a>
        					</td>
					</tr>
			<?php
